...
@router.post("/auth", response_model=TelegramAuthResponse, summary="Verify Telegram Mini App auth data")
async def authenticate(payload: TelegramAuthRequest) -> TelegramAuthResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Malformed init data") from exc

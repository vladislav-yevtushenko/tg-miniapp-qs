...
class Settings(BaseSettings):
    ...
    storage_backend: str = Field(
        default="s3", validation_alias="STORAGE_BACKEND"
    )  # "s3", "minio", or "supabase"
    ...
    s3_bucket_name: str = Field(
        default="marketplace-photos", validation_alias="S3_BUCKET_NAME"
    )
    s3_region: str = Field(default="us-east-1", validation_alias="S3_REGION")
    s3_public_url_base: str | None = Field(
        default=None, validation_alias="S3_PUBLIC_URL_BASE"
    )
    ...
    supabase_bucket: str | None = Field(
        default="listing-photos", validation_alias="SUPABASE_BUCKET"
    )
